//
//  GDPCompareViewController.swift
//  BaseMVVM
//
...
class GDPCompareViewController: ViewController, ChartViewDelegate {
    ...
    override func bindViewModel() {
        super.bindViewModel()
        guard let viewModel = self.viewModel as? GDPCompareViewModel else { return }
        
        let input = GDPCompareViewModel.Input()
    ...
    private func createListLineDataSet(i: Int, j: Int, dataSets: inout [LineChartDataSet], dataEntries: inout [ChartDataEntry], color: UIColor ){
        ...
        chartDataSet.colors = [UIColor.App.unemployment_female2]
        chartDataSet.circleColors = [UIColor.App.unemployment_female2]
    ...
    private func createListCompareLineDataSet(i: Int, j: Int, dataSets: inout [LineChartDataSet], dataEntries: inout [ChartDataEntry], color: UIColor ){
        // Chỉ số giá tiêu dùng chung
        let dataEntry = ChartDataEntry(x: Double(i), y: (iipDatas[j].value[i] + Double(i+2)), data: months[i] as AnyObject)
        dataEntries.append(dataEntry)
        ...
        chartDataSet.circleColors = [UIColor.App.unemployment_male2]
